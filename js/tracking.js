      ...
      if (packageObj.status[i].url !== null)
      {
        $("#statusTable").find('tbody')
          .append($('<tr>')
            ...
            .append($('<td>').text((i > 0) ? moment.duration(packageObj.status[i - 1].date.diff(packageObj.status[i].date)).humanize() : (packageObj.delivered) ? '-' : packageObj.status[i].date.fromNow()))
